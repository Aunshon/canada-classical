  ...
  <tbody>
        @foreach ($experienceCanada as $item)
            <tr>
                ...
                <td>
                    <img src="{{asset('uploads/canada')}}/{{$item->photo}}" alt="NO" width="150">
                </td>
                <td>
                    <textarea cols="30" rows="10" disabled>{{$item->dis}}</textarea>
                </td>
                <td><a href="{{url('deleteexperienceCanada')}}/{{$item->id}}" class="btn btn-danger">Delete</a></td>
...
<div class="container-fluid">
    ...
    <form class="form-inline" enctype="multipart/form-data" action="{{route('saveexperienceCanada')}}" method="POST">
        @csrf
