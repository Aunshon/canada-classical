  ...
  <tbody>
        @foreach ($Partner as $item)
            <tr>
                ...
                <td>
                    <img src="{{asset('uploads/partner')}}/{{$item->photo}}" alt="NO" width="150">
                </td>
                <td>
                    <textarea cols="30" rows="10" disabled>{{$item->dis}}</textarea>
                </td>
                <td>
                    <td><a href="{{url('deletepartner')}}/{{$item->id}}" class="btn btn-danger">Delete</a></td>
...
<div class="container-fluid">
    ...
    <form class="form-inline" enctype="multipart/form-data" action="{{route('savepartner')}}" method="POST">
        @csrf
